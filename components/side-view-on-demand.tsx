...
export function SideViewOnDemand({
  // userID,
  toolInvocation,
  data,
  selectedTemplate,
  ...
  const {
    args,
    result,
  }: { args: Record<string, any>; result?: CodeExecResult } = toolInvocation;
  let latestData: any = {
    state: "initial",
  };
  if (data.length > 0) {
      ...
      >
        <div className="w-full p-2 grid grid-cols-3 items-center justify-end bg-[#FAFAFA] rounded-t-lg border-b border-[#FFE7CC]">
          <div className="flex justify-start">
            {latestData.state === "running" && (
              <LoaderCircle className="h-4 w-4 text-black/15 animate-spin" />
            )}
          </div>
        ...
        {toolInvocation && (
          <div className="w-full flex-1 flex flex-col items-start justify-start overflow-y-auto">
            ...
            >
              <ArtifactView template={result?.template} result={result} />
            </TabsContent>
          </div>
        )}
      </Tabs>
